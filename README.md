...
- Intake actuator、Shooter actuator、Turretの通常位置制御は、limit switchまたはabsolute encoderによるhomingが実装されるまで出力を拒否します。自動Hardware Self-Testでも動かしません。別途armした手動診断だけが、referenceを発行せず、選択した1台を一方向3%・最大0.35秒だけ動かします。
...
| Maintenance（Testのみ・fallbackなし） | Create + L1 / R1 | 選択済み未homing actuatorのnegative / positive 3%診断pulse |
| Maintenance（未接続時のみDriverへfallback、Testのみ） | Options + Square/Cross/Circle/Triangle | climber left+/left-/right+/right- 0.35秒診断pulse |

未homing actuator診断はDisabledでTest modeを選び、Diagnostics / SetupでID30/38/39とNegative/Positiveをそれぞれexact-one選択し、Physical ClearanceとBrushless Motor Typeを確認してからfresh Armを立てます。その後Test Enableし、Maintenance controllerを一度全releaseしてからCreateと、snapshot済み方向に対応するL1またはR1だけを保持します。1回のArmで1 pulseだけ実行し、開始前と終了後にSPARKのzero-output evidenceを確認します。途中releaseでもcommandは停止確認までrequirementsを保持し、外部cancel時だけ`STOP_REQUESTED`までを事実どおり表示します。Hardware Self-Test Armとの同時armは両方拒否します。
...
3. Hardware Self-Testの`GLOBAL_START` stop barrierがCONFIRMEDになってから、homing済みまたは連続回転機構の低出力motion evidenceを採る。ID30/38/39は自動試験対象外。
4. Feeder ID32は物理詰まり・電源枝を解消してから、compile-time controlled retestを明示的に有効化して3%だけ再試験する。
5. ID30/38/39は上記の手動診断で1台・一方向ずつpolarity evidenceを採った後もreference扱いにせず、limit switchまたはabsolute referenceを実装してからhomingし、位置方向・soft limitを確認する。
